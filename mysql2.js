...
var expapp=express();
const mysqlconn=mysql.createConnection({host:'localhost',user:'root',password:'root',database:'mysql'});
...
function getAllEmployee(request,response){
    req=request;
    res=response;
    mysqlconn.query('select emp_name ,salary,landline from employee where emp_name="Swaroop"',printEmployees);
    for(i=0;i<emplist.length;i++){
    var emp=emplist[i];
    var str =
    "<html> <body> <table border=2> <head><tr><th><b>Emp_Name</b></th><th><b>Salary</b></th><th><b>Landline</b></th></tr></head>";
  str += "<body><tr><td>" + emp.emp_name + "</td><td>" + emp.salary + "</td><td>" + emp.landline + "</td></tr>";
  str +='</body></html>';
...
function getUpdate(request,response){
    req=request;
    res=response;
    mysqlconn.query('update employee set salary=35000 where salary=45000');
    mysqlconn.query('select *from employee',printEmployees);
...
function getDelete(request,response){
    req=request;
    res=response;
    mysqlconn.query('delete from employee where emp_id=100',printEmployees);
    console.log("deleted 103");
}
